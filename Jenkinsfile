pipeline {
    ...
    stages {
      stage ('Build') {
        ...
        stage ('SCA') {
            steps {
                sh 'mvn clean deploy org.owasp:dependency-check-maven:check'
        ...
        stage ('ZAP'){
            steps {
                ...
                script{
                    ...
                    sh '${DOCKER_EXEC} run --add-host="localhost:127.0.0.1" -v /home/kali/DevSecOps/zap:/zap/wrk/:rw --rm -i owasp/zap2docker-stable zap-full-scan.py -t "http://demo.testfire.net/" -I -r zap_baseline_report2.html -l PASS'
                }
            }
        }
        stage('Scan Docker'){
